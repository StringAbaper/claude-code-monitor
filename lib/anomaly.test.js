...
// ──────────────────────────────────────────────
// Phase 0 boundary / regression additions
// ──────────────────────────────────────────────

// getGlobalState fresh-start: no events, no window, zero everything
test("getGlobalState on a fresh reset reports empty window state", () => {
  const st = a.getGlobalState();
  assert.equal(st.window5hTotal, 0);
  assert.equal(st.windowStart, null);
  assert.equal(st.windowEnd, null);
  assert.equal(st.msUntilReset, null);
  assert.equal(st.eventCount, 0);
  assert.equal(st.level, "ok");
});

// Weighted delta math — explicit regression guard. If anyone tweaks the
// weights without updating tests, this breaks loudly.
test("weightedDelta applies Sonnet-ish pricing ratios (input=1, output=5, cacheRead=0.1, cacheWrite=1.25)", () => {
  const s = freshSession();
  const t = Date.now() - 60_000;
  // input 100 → weighted 100
  // output 100 → weighted 500
  // cached 100 → weighted 10
  // cacheWrite 100 → weighted 125
  // total 735
  feed(s, entry(t, 100, 100, 100, 100));
  assert.equal(a.getGlobalState().window5hTotal, 735);
});

// Debounce: a second anomaly of the same kind within the debounce window
// must not generate a duplicate alert.
test("alerts are debounced within ALERT_DEBOUNCE_MS per session+anomaly", () => {
  const s = freshSession();
  const base = Date.now() - 60_000;
  // First cacheMissSpike — should fire.
  const e1 = entry(base, 0, 0, 1000, 50000);
  const r1 = a.check(s, e1);
  s.usageHistory.push(e1);
  assert.ok(r1.alerts.some(al => al.id === "cacheMissSpike"), "first fires");

  // Second cacheMissSpike 5 seconds later — should be debounced away
  // (ALERT_DEBOUNCE_MS is 60s).
  const e2 = entry(base + 5_000, 0, 0, 2000, 100000);
  const r2 = a.check(s, e2);
  s.usageHistory.push(e2);
  assert.ok(!r2.alerts.some(al => al.id === "cacheMissSpike"), "second debounced");
  // entry.anomalies array still records it (UI dot marker) — only the
  // alerts array (toast/sound notification) is debounced.
  assert.ok(e2.anomalies.includes("cacheMissSpike"), "still recorded on entry");
});

// Peak hour burn — positive path
test("peakHourBurn fires during configured peak window with high burn over baseline", () => {
  // Use a timezone + window that will definitely be 'peak' right now.
  // Strategy: pick a window that spans the entire 24h so isPeakHour() is
  // always true regardless of the machine's local TZ.
  a.setConfig({ peakHoursStart: 0, peakHoursEnd: 23 });
  const s = freshSession();
  const base = Date.now() - 30_000;
  // Seed a moderate baseline: 10 evenly-spaced entries of 100 tokens each
  // over the past 11 minutes, building up perMinuteEMA.
  for (let i = 0; i < 10; i++) {
    feed(s, entry(base - (10 - i) * 60_000, 100 * (i + 1), 0, 0, 0));
  }
  // Now slam a large entry that bumps the 10m burn well above EMA * 1.5
  const e = entry(base, 100_000, 0, 0, 0);
  const r = a.check(s, e);
  s.usageHistory.push(e);
  // burnRateSpike or peakHourBurn may both fire; we only assert the
  // detector we're exercising.
  assert.ok(
    e.anomalies.includes("peakHourBurn") || e.anomalies.includes("burnRateSpike"),
    `expected a burn-family anomaly, got ${JSON.stringify(e.anomalies)}`
  );
});

// Peak hour burn — negative path outside the peak window
test("peakHourBurn does NOT fire outside the configured peak window", () => {
  // Set a peak window that definitely does NOT include "right now" in any
  // reasonable TZ by making it a single impossible hour. peakHoursStart=25
  // is clamped to 23 by setConfig, so instead we pick start=end=1 which
  // means the window is empty.
  a.setConfig({ peakHoursStart: 1, peakHoursEnd: 1 });
  const s = freshSession();
  const base = Date.now() - 30_000;
  for (let i = 0; i < 10; i++) {
    feed(s, entry(base - (10 - i) * 60_000, 100 * (i + 1), 0, 0, 0));
  ...
  const e = entry(base, 100_000, 0, 0, 0);
  a.check(s, e);
  assert.ok(!e.anomalies.includes("peakHourBurn"));
});

// Resume flag lifecycle: noteResume() + one check consumes the flag so a
// second check sees afterResume === false.
test("resumeFlag is consumed after the first check following noteResume", () => {
  const s = freshSession();
  // Build baseline first
  for (let i = 0; i < 5; i++) {
    feed(s, entry(Date.now() - (10 - i) * 60_000, 100, 0, 0, 1000));
  }
  a.noteResume(s);

  const e1 = entry(Date.now() - 30_000, 200, 0, 0, 2000);
  a.check(s, e1);
  s.usageHistory.push(e1);
  assert.equal(e1.afterResume, true, "first check sees the flag");

  const e2 = entry(Date.now() - 10_000, 300, 0, 0, 2500);
  a.check(s, e2);
  assert.equal(e2.afterResume, false, "second check no longer sees the flag");
});
