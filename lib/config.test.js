...
test("showContextBudget defaults to true and rejects truthy strings", () => {
  assert.equal(c.get().showContextBudget, true);
  c.set({ showContextBudget: "yes" });
  assert.equal(c.get().showContextBudget, true, "string rejected, default kept");
  c.set({ showContextBudget: false });
  assert.equal(c.get().showContextBudget, false);
});

test("showSkillAnalytics defaults to true and is strict-boolean", () => {
  assert.equal(c.get().showSkillAnalytics, true);
  c.set({ showSkillAnalytics: 1 });
  assert.equal(c.get().showSkillAnalytics, true, "number rejected");
  c.set({ showSkillAnalytics: false });
  assert.equal(c.get().showSkillAnalytics, false);
});

test("requireLogin defaults to true and is strict-boolean", () => {
  assert.equal(c.get().requireLogin, true);
  c.set({ requireLogin: "no" });
  assert.equal(c.get().requireLogin, true, "string rejected");
  c.set({ requireLogin: false });
  assert.equal(c.get().requireLogin, false);
});

test("skillRepoPath defaults to empty string, accepts valid strings, rejects non-strings", () => {
  assert.equal(c.get().skillRepoPath, "");
  c.set({ skillRepoPath: "D:/Projects/CCB-skills" });
  assert.equal(c.get().skillRepoPath, "D:/Projects/CCB-skills");
  c.set({ skillRepoPath: 42 });
  assert.equal(c.get().skillRepoPath, "D:/Projects/CCB-skills", "number rejected, previous kept");
});
