    ...
    public static string Focus(string termsStr, string pidsStr) {
        ...
        EnumWindows(delegate(IntPtr hWnd, IntPtr lParam) {
            ...
            if (pidRank >= 0) {
                ...
                // (almost certainly) the hosting terminal/IDE. Nearest ancestor
                // wins; a title match on top breaks ties between windows of the
                // same process (e.g. several VS Code windows share one pid).
                score = 1000 - pidRank + (matched ? 100 : 0);
            } else {
                if (!matched) return true;
                // Title-only fallback. Never focus Explorer on a title match:
                // its title is just the folder path, not a session host.
                string pname = "";
...
// Build search terms from session data. The launch directory comes first: an
// editor window is titled after the workspace it opened, so a session that
// has cd'd elsewhere is still found by the project it belongs to.
function buildSearchTerms(session) {
  const terms = [session.projectRoot, session.project].filter(Boolean);
  for (const dir of [session.rootCwd, session.cwd]) {
    if (!dir) continue;
    terms.push(dir);
    terms.push(dir.replace(/\\/g, "/"));
