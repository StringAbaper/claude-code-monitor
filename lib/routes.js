...
const { shouldIntercept, describeActivity, describeApproval, processEvent } = require("./tools");
...
// The question has been answered, so the session is no longer waiting on a
// human — it is running the tool. Without this the sidebar tile and the tab
// badge stay flagged "Permission" until PostToolUse finally arrives, which
// for a long command is minutes after the answer.
//
// Only clears once nothing is pending for that session: parallel tool calls
// can raise more than one card at a time. An approval cancelled because its
// hook died is deliberately not an answer — Claude Code is still holding its
// own prompt open, so the session really is still waiting.
function clearWaitingOnAnswer(sessionId) {
  const session = store.getSession(sessionId);
  if (!session || session.status !== "waiting_permission") return;
  if (store.getPendingApprovals().some((p) => p.sessionId === sessionId)) return;
  session.status = "working";
  session.activityText = session.currentTool
    ? describeActivity(session.currentTool, session.toolDetail)
    : "Working";
}
...
function register(app, broadcast) {
  ...
  app.post("/api/event", requireToken, (req, res) => {
    try {
      ...
      if (remoteApprovalEnabled && shouldIntercept(data)) {
        ...
        // Label the card with the session's project, not with whatever
        // directory the tool happens to be run from.
        const known = store.getSession(data.session_id);
        const rootDir = data.project_dir || (known && known.rootCwd) || data.cwd || "";
        store.setApproval(approvalId, {
          id: approvalId,
          sessionId: data.session_id,
          project: path.basename(rootDir) || rootDir || "Unknown",
          cwd: data.cwd || "",
  ...
  app.post("/api/pending/:id/decide", requireToken, (req, res) => {
    ...
    clearWaitingOnAnswer(a.sessionId);
  ...
  app.post("/api/settings", requireToken, (req, res) => {
    ...
    if (changed) {
      ...
      if (autoApproveEnabled) {
        ...
        for (const sessionId of touched) clearWaitingOnAnswer(sessionId);
      }
      broadcast(store.getFullState());
    }
