...
let updateChannel = "stable"; // 'stable' | 'beta'
let uiSkin = "default"; // 'default' | 'linear' | 'sentry' | 'raycast'
const ALLOWED_UI_SKINS = new Set(["default", "linear", "sentry", "raycast"]);
let apiToken = null;
...
function loadConfig() {
  try {
    if (fs.existsSync(CONFIG_FILE)) {
      ...
      if (data.updateChannel === "stable" || data.updateChannel === "beta") updateChannel = data.updateChannel;
      if (typeof data.uiSkin === "string" && ALLOWED_UI_SKINS.has(data.uiSkin)) uiSkin = data.uiSkin;
...
function getFullState() {
  return {
    ...
    settings: { remoteApprovalEnabled, autoApproveEnabled, stoppedCleanupMinutes, showTokenChart, autoArchiveStopped, autoUpdateEnabled, updateChannel, uiSkin },
...
function getConfig() {
  return { remoteApprovalEnabled, autoApproveEnabled, stoppedCleanupMinutes, showTokenChart, autoArchiveStopped, autoUpdateEnabled, updateChannel, uiSkin };
}
...
function setConfig(updates) {
  ...
  if (updates.updateChannel === "stable" || updates.updateChannel === "beta") {
    updateChannel = updates.updateChannel;
    changed = true;
  }
  if (typeof updates.uiSkin === "string" && ALLOWED_UI_SKINS.has(updates.uiSkin)) {
    uiSkin = updates.uiSkin;
    changed = true;
  }
