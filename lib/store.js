...
function verifyPasswordHash(pw, stored) {
  ...
  if (!stored.startsWith("pbkdf2:")) {
    return crypto.createHash("sha256").update(pw).digest("hex") === stored;
  }
...
// Auto-cleanup: remove stopped sessions older than threshold
const STOPPED_CLEANUP_MS = 24 * 60 * 60_000; // 24 hours
function cleanupOldSessions() {
  const now = Date.now();
  let changed = false;
  for (const [id, s] of sessions) {
    if (s.status === "stopped" && now - s.lastActivity > STOPPED_CLEANUP_MS) {
      sessions.delete(id);
      changed = true;
    }
  }
