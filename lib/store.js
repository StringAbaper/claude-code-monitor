  ...
  try {
    if (fs.existsSync(SESSIONS_FILE)) {
      ...
      for (const s of data) {
        ...
        // Saved before the project root was tracked: the last cwd we knew
        // is the only root we can offer them.
        if (!s.rootCwd) {
          s.rootCwd = s.cwd || "";
          s.projectRoot = s.project || "Unknown";
        }
        sessions.set(s.id, s);
      }
...
function getOrCreateSession(id, cwd, hostname) {
  if (!sessions.has(id)) {
    sessions.set(id, {
      ...
      // Where the session was launched. Seeded from the first cwd we see
      // and then held; processEvent replaces it the moment a hook reports
      // the real project directory.
      rootCwd: cwd || "",
      projectRoot: cwd ? path.basename(cwd) : "Unknown",
