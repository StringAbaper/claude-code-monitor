...
// Name a session after the directory it was launched in.
function setProjectRoot(session, dir) {
  session.rootCwd = dir;
  session.projectRoot = path.basename(dir) || dir;
}

// ──────────────────────────────────────────────
// Completion debounce
// ──────────────────────────────────────────────
// `Stop` fires whenever the main agent yields the turn — not only when the
// whole task is done. It also fires when Claude pauses to ask a question,
// between a follow-up cycle, when a background task hands control back, or
// when a blocking Stop hook is about to make Claude keep going. Sounding a
// "task complete" chime on every Stop therefore cries wolf: it rings while
// Claude is still working. So we hold the completion alert for a short grace
// window and cancel it if any activity for that session arrives first — a
// genuine finish is a Stop with nothing after it. The session's own state
// still flips to idle immediately; only the notification waits.
let COMPLETE_DEBOUNCE_MS = Number(process.env.CLAUDE_MONITOR_COMPLETE_DEBOUNCE_MS) || 7_000;
const pendingComplete = new Map(); // session_id → timer

function cancelPendingComplete(sessionId) {
  const timer = pendingComplete.get(sessionId);
  if (timer) {
    clearTimeout(timer);
    pendingComplete.delete(sessionId);
  }
}

function scheduleCompleteAlert(session, store, broadcast, detail) {
  cancelPendingComplete(session.id);
  const timer = setTimeout(() => {
    pendingComplete.delete(session.id);
    const payload = store.getFullState();
    payload.alert = {
      type: "complete",
      session: { id: session.id, project: session.projectRoot || session.project, cwd: session.cwd },
      detail,
    };
    broadcast(payload);
  }, COMPLETE_DEBOUNCE_MS);
  // Never let a pending chime hold the process open on shutdown.
  if (typeof timer.unref === "function") timer.unref();
  pendingComplete.set(session.id, timer);
}
...
function processEvent(data, store, broadcast) {
  ...
  // Any event that is not itself a Stop means the session is doing something
  // again, so a completion alert still sitting in its grace window was
  // premature — drop it before it rings.
  if (event_type !== "Stop") cancelPendingComplete(session_id);
  ...
  // The launch directory, which — unlike cwd — stays put when Claude cds
  // into a subdirectory or out of the project entirely. The hook's
  // `project_dir` is authoritative and may correct an earlier guess; with
  // no hook to tell us, the first cwd we ever saw is the best we have, and
  // it sticks rather than following the session around.
  if (typeof data.project_dir === "string" && data.project_dir && data.project_dir.length <= 512) {
    setProjectRoot(session, data.project_dir);
  } else if (!session.rootCwd && cwd) {
  ...
  if (isPermissionAlert) {
    // A prompt needs the user now; it also supersedes any completion chime
    // still waiting out its grace window for this session.
    cancelPendingComplete(session_id);
    payload.alert = {
      type: "permission",
      session: { id: session.id, project: session.projectRoot || session.project, cwd: session.cwd },
    };
  } else if (isComplete) {
    // Defer the completion alert instead of firing it inline. The state
    // update (idle status, usage, any anomaly alert) still goes out now.
    scheduleCompleteAlert(session, store, broadcast, event.detail);
  }
  broadcast(payload);
}
...
module.exports = {
  ...
  processEvent,
  // Test hook: lets the suite shrink the completion grace window so the
  // debounce can be exercised without a real 7-second wait.
  _setCompleteDebounceMs(ms) { COMPLETE_DEBOUNCE_MS = ms; },
};
