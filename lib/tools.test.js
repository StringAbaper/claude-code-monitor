...
test("Stop flips the session to idle immediately but holds the complete alert", () => {
  const store = fakeStore();
  const alerts = [];
  t._setCompleteDebounceMs(50);
  t.processEvent({ event_type: "Stop", session_id: "s1" }, store, (p) => { if (p.alert) alerts.push(p.alert); });
  // State is applied now; only the notification waits.
  assert.equal(store.getSession("s1").status, "idle");
  assert.equal(alerts.length, 0);
});

test("a lone Stop fires the complete alert once its grace window elapses", async () => {
  const store = fakeStore();
  const alerts = [];
  t._setCompleteDebounceMs(20);
  t.processEvent({ event_type: "Stop", session_id: "s1", cwd: "/p" }, store, (p) => { if (p.alert) alerts.push(p.alert); });
  await new Promise((r) => setTimeout(r, 50));
  assert.equal(alerts.length, 1);
  assert.equal(alerts[0].type, "complete");
  assert.equal(alerts[0].session.id, "s1");
});

test("activity within the grace window cancels the premature complete alert", async () => {
  const store = fakeStore();
  const alerts = [];
  t._setCompleteDebounceMs(50);
  const run = (e) => t.processEvent(e, store, (p) => { if (p.alert) alerts.push(p.alert); });
  run({ event_type: "Stop", session_id: "s1" });
  await nextTick(); // still inside the window
  run({ event_type: "PreToolUse", session_id: "s1", tool_name: "Bash", tool_input: {} });
  await new Promise((r) => setTimeout(r, 80));
  // The Stop was a turn boundary, not a finish: no chime.
  assert.equal(alerts.filter((a) => a.type === "complete").length, 0);
});

test("a permission prompt within the grace window supersedes the complete alert", async () => {
  const store = fakeStore();
  const alerts = [];
  t._setCompleteDebounceMs(50);
  const run = (e) => t.processEvent(e, store, (p) => { if (p.alert) alerts.push(p.alert); });
  ...
  run({ event_type: "PermissionRequest", session_id: "s1", tool_name: "Bash", tool_input: {} });
  await new Promise((r) => setTimeout(r, 80));
  assert.deepEqual(alerts.map((a) => a.type), ["permission"]);
});

test("a fresh Stop restarts the grace window rather than firing twice", async () => {
  const store = fakeStore();
  const alerts = [];
  t._setCompleteDebounceMs(40);
  const run = (e) => t.processEvent(e, store, (p) => { if (p.alert) alerts.push(p.alert); });
  run({ event_type: "Stop", session_id: "s1" });
  await new Promise((r) => setTimeout(r, 25)); // not yet fired
  run({ event_type: "Stop", session_id: "s1" });
  await new Promise((r) => setTimeout(r, 80));
  assert.equal(alerts.filter((a) => a.type === "complete").length, 1);
});
