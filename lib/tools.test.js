...
// ──────────────────────────────────────────────
// processEvent: Skill PreToolUse → appendSkillCall
// ──────────────────────────────────────────────
test("processEvent records a skill call on PreToolUse with tool_name=Skill", () => {
  // We need a fake store that captures appendSkillCall invocations.
  // Mirrors the real lib/store.js shape just enough to satisfy
  // processEvent's call sites.
  const calls = [];
  const fakeStore = {
    MAX_RECENT_EVENTS: 30,
    MAX_USAGE_HISTORY: 100,
    getOrCreateSession(id, cwd, hostname) {
      return {
        id,
        cwd: cwd || "",
        project: "test",
    ...
    appendSkillCall(sessionId, call) { calls.push({ sessionId, call }); },
    getFullState() { return { type: "update", sessions: [], pendingApprovals: [], settings: {}, machineNames: {}, serverHostname: "test" }; },
  };
  t.processEvent({
    event_type: "PreToolUse",
    session_id: "test-session",
    tool_name: "Skill",
    tool_input: { skill: "ccb-plan", args: "do a thing" },
  }, fakeStore, () => {});
  assert.equal(calls.length, 1, "appendSkillCall called once");
  assert.equal(calls[0].sessionId, "test-session");
  assert.equal(calls[0].call.name, "ccb-plan");
  // args is intentionally NOT recorded (Q1 decision)
  assert.equal(calls[0].call.args, undefined);
});

test("processEvent does NOT record a skill call when tool_name is not Skill", () => {
  const calls = [];
  const fakeStore = {
    MAX_RECENT_EVENTS: 30,
    MAX_USAGE_HISTORY: 100,
    getOrCreateSession() { return { id: "x", recentEvents: [], usage: {}, usageHistory: [], skill_calls: [], status: "working", lastActivity: 0, startedAt: 0, eventCount: 0 }; },
    appendSkillCall(sessionId, call) { calls.push({ sessionId, call }); },
    getFullState() { return { type: "update", sessions: [], pendingApprovals: [], settings: {}, machineNames: {}, serverHostname: "test" }; },
  };
  t.processEvent({
