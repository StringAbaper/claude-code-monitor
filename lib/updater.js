...
const INDEX_LOCK_PATH = path.join(ROOT_DIR, ".git", "index.lock");
const INDEX_LOCK_STALE_MS = 60_000; // a lock older than this is considered orphaned

// Record the module load time so we can identify index.lock files that
// must have been created by a previous (now-dead) process.
const MODULE_LOADED_AT = Date.now();

// Self-heal orphan .git/index.lock left behind by a crashed prior git
// operation. Only deletes the lock when BOTH:
//   (1) its mtime is older than this process started, AND
//   (2) its age exceeds INDEX_LOCK_STALE_MS (60s)
// so a concurrent legitimate git process on the same repo is never
// interrupted. Returns true if a stale lock was removed.
function clearStaleIndexLock() {
  try {
    const stat = fs.statSync(INDEX_LOCK_PATH);
    const now = Date.now();
    if (stat.mtimeMs < MODULE_LOADED_AT && (now - stat.mtimeMs) > INDEX_LOCK_STALE_MS) {
      fs.unlinkSync(INDEX_LOCK_PATH);
      console.log("  [updater] Removed stale .git/index.lock from a prior crashed git process");
      return true;
    }
  } catch {
    // ENOENT (no lock) is the normal case
  }
  return false;
}
...
function applyUpdate() {
  ...
  try {
    // Before any git operation, opportunistically clear a stale index
    // lock from a crashed prior process. This was a repeated failure mode
    // on remote servers that required manual SSH + rm -f .git/index.lock.
    clearStaleIndexLock();

    // Stash any local changes. If stash fails with "could not write index"
    // we make ONE more attempt after trying to clear the lock (even if it
    // wasn't stale the first time — a race window just closed).
    const tryStash = () => {
      try {
    ...
    let stashErr = tryStash();
    if (stashErr && /could not write index|index\.lock/i.test(stashErr.stderr || stashErr.message || "")) {
      // Force-clear the lock: the stash already failed, any concurrent
      // git process would have deadlocked by now. Retry once.
      try { fs.unlinkSync(INDEX_LOCK_PATH); } catch {}
      console.log("  [updater] Cleared index.lock after git stash failure; retrying once");
      stashErr = tryStash();
    }
    if (stashErr) {
      // Only ignore "nothing to stash" — anything else is fatal.
    ...
    // shell:true is required on Windows because npm is npm.cmd, which
    // execSync without a shell cannot resolve via PATH.
    // This step is non-fatal: the git pull already succeeded, so even if
    // npm install fails (PATH issues, offline, etc.) the new code is on
    // disk and will run on next restart. We surface it as a warning, not
    // a rollback — rolling back would lose the user's pulled commits.
    ...
    if (pullOutput.includes("package.json") || pullOutput.includes("package-lock.json")) {
      try {
        ...
        installWarning = "npm install failed (" + (installErr.message || "unknown") + "). Code is updated; run 'npm install' manually if dependencies changed.";
      }
    }
    ...
    updateState.lastError = installWarning;

    return { ok: true, version: updateState.currentVersion, needsRestart: true, warning: installWarning };
  } catch (err) {
...
module.exports = {
  ...
  startPeriodicCheck,
  stopPeriodicCheck,
  cmpSemver, // exported for unit testing; pure function
};
